...
class variable_set
{
        ...
        motor::graphics::type const t, motor::graphics::type_struct const ts ) noexcept
    {
...

        {
            ...
            case motor::graphics::type::tchar:
                var = this_t::from_type_struct< char_t >( name, ts );
                break;
            case motor::graphics::type::tuchar:
                var = this_t::from_type_struct< uchar_t >( name, ts );
                break;
            case motor::graphics::type::tshort:
                var = this_t::from_type_struct< short_t >( name, ts );
                break;
            case motor::graphics::type::tushort:
                var = this_t::from_type_struct< ushort_t >( name, ts );
                break;
            case motor::graphics::type::tint:
                var = this_t::from_type_struct< int_t >( name, ts );
                break;
            case motor::graphics::type::tuint:
                var = this_t::from_type_struct< uint_t >( name, ts );
                break;
            case motor::graphics::type::tfloat:
                var = this_t::from_type_struct< float_t >( name, ts );
                break;
            case motor::graphics::type::tdouble:
                var = this_t::from_type_struct< double_t >( name, ts );
                break;
            case motor::graphics::type::tbool:
                var = this_t::from_type_struct< bool_t >( name, ts );
                break;
            default:
                break;
            }
            ...
            if( var == nullptr )
            {
                char buffer[ 1024 ];
                std::snprintf(
                    buffer, 1024, "[variable_set] : type mismatch for %s", name.c_str() );
                motor::log::global_t::error( buffer );
                return var;
            }
  ...
  private: // texture variable
    ...
    //***************************************************************************************
    motor::graphics::texture_variable_t * find_texture_variable_us(
        char const * const name ) const noexcept
    {
        size_t i = size_t( -1 );
        while( ++i < _textures.size() && _textures[ i ].name != name );
        return ( i == _textures.size() )
                   ? nullptr
  ...
  private: // array variable
    ...
    //***************************************************************************************
    motor::graphics::array_variable_t * find_array_variable_us(
        char const * const name ) const noexcept
    {
        size_t i = size_t( -1 );
  ...
  public: // streamout array vars
    ...
    using for_each_data_var_funk_t =
        std::function< void_t( motor::string_in_t, motor::graphics::ivariable_ptr_t ) >;
    ...
    using for_each_texture_var_funk_t = std::function< void_t(
        motor::string_in_t, motor::graphics::data_variable< motor::string_t > * ) >;
  ...
  private:
        ...
        motor::string_cref_t /*name*/, motor::graphics::type_struct const ts ) noexcept
    {
        ...
        case motor::graphics::type_struct::mat2:
            return motor::memory::global_t::alloc(
                motor::graphics::data_variable< motor::math::matrix2< T > >(
                    motor::math::matrix2< T >().identity() ),
                "mat2 variable" );
        case motor::graphics::type_struct::mat3:
            return motor::memory::global_t::alloc(
                motor::graphics::data_variable< motor::math::matrix3< T > >(
                    motor::math::matrix3< T >().identity() ),
                "mat2 variable" );
        case motor::graphics::type_struct::mat4:
            return motor::memory::global_t::alloc(
                motor::graphics::data_variable< motor::math::matrix4< T > >(
                    motor::math::matrix4< T >().identity() ),
                "mat2 variable" );
        default:
            break;
        }
...
template < class T >
static motor::graphics::data_variable< T > * any_variable(
    variable_set_ptr_t vs, motor::string_cref_t name ) noexcept
{
...
any_variable< motor::graphics::texture_variable_data >(
    variable_set_ptr_t vs, motor::string_cref_t name ) noexcept
{
...
any_variable< motor::graphics::array_variable_data >(
    variable_set_ptr_t vs, motor::string_cref_t name ) noexcept
{
...
any_variable< motor::graphics::streamout_variable_data >(
    variable_set_ptr_t vs, motor::string_cref_t name ) noexcept
{
