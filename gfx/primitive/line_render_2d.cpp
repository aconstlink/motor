...
//**********************************************************************************************************
line_render_2d::line_render_2d( this_rref_t rhv )  noexcept
{
    *this = std::move( rhv ) ;
}

//**********************************************************************************************************
line_render_2d::this_ref_t line_render_2d::operator = ( this_rref_t rhv ) noexcept 
{
...
//**********************************************************************************************************
void_t line_render_2d::init( motor::string_cref_t name ) noexcept 
{
    _name = name ;
    ...
    // geometry configuration
    {
        ...
        _go = motor::graphics::geometry_object_t( name + "_lines",
    ...
    {
        motor::graphics::shader_object_t sc( name + "_shader" ) ;
    ...
    // the render object
    {
        motor::graphics::render_object_t rc = motor::graphics::render_object_t( name + "_render_object" ) ;
    ...
    motor::math::vec2f_cref_t p1, motor::math::vec4f_cref_t color ) noexcept
{
...

    {
        ...
        layer->lines[ layer->lines.resize_by( 1, 1000 ) ] = this_t::line_t { { p0, p1 }, color } ;
...
//**********************************************************************************************************
void_t line_render_2d::draw_circle( size_t const l, size_t const s, motor::math::vec2f_cref_t p0, float_t const r, motor::math::vec4f_cref_t color ) noexcept 
{
...
//**********************************************************************************************************
void_t line_render_2d::configure( motor::graphics::gen4::frontend_mtr_t fe ) noexcept 
{
...
void_t line_render_2d::prepare_for_rendering( void_t ) noexcept 
{
    ...
    // 1. copy data
    {
        ...
        for( size_t i=0; i<_layers.size(); ++i )
        {
            ...
            {
                #if 1
                motor::concurrent::parallel_for<size_t>( motor::concurrent::range_1d<size_t>(0, lines.size()),
                    [&]( motor::concurrent::range_1d<size_t> const & r )
                {
                    for( size_t l=r.begin(); l<r.end(); ++l )
                    {
                        size_t const idx = lstart + l ;
                        _ao.data_buffer().update< motor::math::vec4f_t >( idx, lines[l].color ) ;
                    }
                } ) ;
                #else
                for( size_t i=0; i<lines.size();++i)
                {
                    ...
                    _ao->data_buffer().update< motor::math::vec4f_t >( idx, lines[i].color ) ;
                }
                #endif
                lstart += lines.size() ;
            }
...
//**********************************************************************************************************
void_t line_render_2d::prepare_for_rendering( motor::graphics::gen4::frontend_mtr_t fe ) noexcept 
{
...
//**********************************************************************************************************
void_t line_render_2d::render( motor::graphics::gen4::frontend_mtr_t fe, size_t const l ) noexcept 
{
...
//**********************************************************************************************************
void_t line_render_2d::add_variable_set( motor::graphics::render_object_ref_t rc ) noexcept 
{
...
//**********************************************************************************************************
void_t line_render_2d::set_view_proj( motor::math::mat4f_cref_t view, motor::math::mat4f_cref_t proj ) noexcept 
{
    _view = view ;
    _proj = proj ;
}
