...
auto tri_down =
    []( ImVec2 const pos, float_t const height, ImU32 const color, ImDrawList * draw_list )
{
    ...
    []( ImVec2 const pos, float_t const height, ImU32 const color, ImDrawList * draw_list )
{
    ...
    ImVec2 const points[] = { pos, pos + ImVec2( h, h ), pos + ImVec2( -h, h ) };
...
bool_t timeline::begin( motor::tool::time_info_ref_t ti ) noexcept
{
    ...
    {
        _zoom = std::max(
            size_t( 1 ), size_t( _max_milli / size_t( ImGui::GetContentRegionAvail().x ) ) );
    }
    ...
    // ImVec2 const scrolling_child_size = ImVec2( 0, ImGui::GetContentRegionAvail().y ) -
    // ImVec2(0.0f, ImGui::GetTextLineHeight()+10) ;
    {
        char buffer[ 1024 ];
    ...
    {
        {
            ...
            draw_list->AddLine( pos + ImVec2( 0.0f, 10.0f ), pos + ImVec2( 0.0f, 10.0f ),
                IM_COL32( 255, 255, 255, 0 ), 1.0f );
        }

        ImGui::SetCursorScreenPos(
            ImGui::GetCursorScreenPos() + ImVec2( this_t::milli_to_pixel( ti.max_milli ), 0.0f ) );

        {
            ImVec2 pos = ImGui::GetCursorScreenPos();
            draw_list->AddLine( pos + ImVec2( 0.0f, 0.0f ), pos + ImVec2( 0.0f, 10.0f ),
    ...
    float_t const mouse_x =
        std::max( 0.0f, std::min( ImGui::GetMousePos().x - ImGui::GetWindowPos().x + scroll_x,
                            float_t( this_t::milli_to_pixel( ti.max_milli ) ) ) );

    // motor::log::global_t::status( motor::to_string(mouse_x) ) ;

    bool_t const mouse_in_cr =
    ...
    if( ImGui::IsKeyDown( ImGuiKey::ImGuiKey_LeftCtrl ) && mouse_in_cr )
    {
        ...
        if( tmp != 0 )
            ImGui::SetScrollX(
                std::max( mouse_x - ImGui::GetContentRegionAvail().x * 0.5f, 0.0f ) );
    }
    ...
    // draw time line
    {
        ...
        ImVec2 const start =
            ImGui::GetCursorScreenPos() + ImVec2( scroll_x - offset_x, top_line_height );
...
// draw big lines
    ...
    // current position marker
    {
        ...
        if( marker_pos > scroll_x && marker_pos < scroll_x + avail.x )
        {
            ...
            ImVec2 const p1 =
                pos + ImVec2( marker_pos, big_line_height + ImGui::GetTextLineHeight() );
...
void_t timeline::end( void_t ) noexcept
{
...

    {
        ...
        ImGui::DragInt(
            buffer, &z, 5.0f, 0, (int_t)_max_milli / (int_t)ImGui::GetContentRegionAvail().x );
...
{
    motor::tool::make_time_string( _time_string_buffer, sizeof( _time_string_buffer ), milli );
