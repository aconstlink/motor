...
class MOTOR_TOOL_API timeline
{
  ...
  public:
    ...
    // returns true if marker is in time range.
    bool_t place_marker( this_t::marker_position const, size_t const milli, ImColor const & color );
  ...
  public:
    ...
    size_t mtp( size_t const m ) const noexcept
    {
        return this_t::milli_to_pixel( m );
    }
    size_t ptm( size_t const p ) const noexcept
    {
        return this_t::pixel_to_milli( p );
    }
