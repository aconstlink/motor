...
rawinput_module::rawinput_module( void_t ) noexcept 
{
...

    {
        _three_device = motor::memory::create_ptr( 
            motor::controls::three_device_t( "Win32 Rawinput Three Button Mouse" ),
            "RawInput Mouse") ;

        _ascii_device = motor::memory::create_ptr( 
            motor::controls::ascii_device_t( "Win32 Rawinput Ascii Keyboard" ),
            "RawInput Keyboard") ;
    }
}
...
//***
void_t rawinput_module::search( motor::controls::imodule::search_funk_t funk ) noexcept
{
    funk( _three_device ) ;
    funk( _ascii_device ) ;
}
    ...
    if( raw->header.dwType == RIM_TYPEMOUSE )
    {
        ...
        if( raw->data.mouse.usFlags == MOUSE_MOVE_RELATIVE )
        {
            bs = ( last_pt.x == 0 && last_pt.y == 0 ) ?
                motor::controls::components::button_state::pressed :
                motor::controls::components::button_state::none ;
        }
