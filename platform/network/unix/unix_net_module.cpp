...
//*****************************************************************
unix_net_module::unix_net_module( void_t ) noexcept
{
}

//*****************************************************************
unix_net_module::unix_net_module( this_rref_t rhv ) noexcept
{
...
//*****************************************************************
motor::network::socket_id_t unix_net_module::create_tcp_server(
    motor::network::create_tcp_server_info_rref_t info_in ) noexcept
{
    if ( info_in.handler == nullptr )
    {
        motor::log::global_t::error("[unix_net_module] : server handler required") ;
    ...
    {
        s = socket( result->ai_family, result->ai_socktype | SOCK_NONBLOCK, result->ai_protocol );
        if ( s == INVALID_SOCKET )
        {
            int const err = errno ;
    ...
    #if 0
    {
        ...
        if ( res != 0 )
        {
            motor::log::global::error( "[unix_net_module::create_tcp_server] : ioctlsocket" ) ;
            motor::log::global::error( "[unix_net_module::create_tcp_server] : WSAGetLastError " +
        ...
        tcpd->t = std::thread( [=] ( void_t )
        {
            ...
            while ( tcpd->running )
            {
                ...
                else if ( s == INVALID_SOCKET )
                {
                    ...
                    {
                        motor::log::global::error( "[unix_net_module::server_thread] : accept" ) ;
                        motor::log::global::error( "[unix_net_module::server_thread] : WSAGetLastError " +
                            motor::to_string(res) ) ;
                    }
                }
                        ...
                        // send
                        {
                            ...
                            while ( true )
                            {
                                ...
                                // do the actual send operation
                                {
                                    ...
                                    {
                                        motor::log::global::error( "[unix_net_module::server_thread] : send" ) ;
                                        motor::log::global::error( "[unix_net_module::server_thread] : WSAGetLastError " +
...
//*****************************************************************
SOCKET unix_net_module::connect_client( motor::network::ipv4::binding_point_host const & bp ) const noexcept
{
    SOCKET s = INVALID_SOCKET ;
    addrinfo * result = NULL ;
    addrinfo * ptr = NULL ;
    ...
    {
        int timeout_ms = 1000 ;
        auto const res = setsockopt( s, SOL_SOCKET, SO_RCVTIMEO, (char_cptr_t) &timeout_ms, sizeof (int) );
        motor::log::global_t::error( res != 0, "[unix_net_module::connect_client] : setsockopt SO_RCVTIMEO" ) ;
    }
    ...
    #if 0
    {
        ...
        motor::log::global_t::error( res != 0, "[unix_net_module::connect_client] : getsockopt SO_MAX_MSG_SIZE" ) ;
    }
    #endif
...
//*****************************************************************
motor::network::socket_id_t unix_net_module::create_tcp_client(
    motor::network::create_tcp_client_info_rref_t info_in ) noexcept
{
    if ( info_in.handler == nullptr )
    {
        motor::log::global_t::error( "[unix_net_module] : client handler required" ) ;
        return motor::network::socket_id_t( -1 ) ;
    }
            ...
            while ( true )
            {
                ...
                // send
                {
                    ...
                    if ( sib != 0 )
                    {
                        ...
                        if ( has_sent != sib )
                        {
                            motor::log::global_t::status( "[unix_net_module::send] : has_sent != num_bytes" ) ;
                            tres = motor::network::transmit_result::failed ;
                        }
                    }
...
//*****************************************************************
void_t unix_net_module::update( void_t ) noexcept
{

}

//*****************************************************************
size_t unix_net_module::create_tcp_client_id( void_t ) noexcept
{
    ...
    _tcp_clients[ id ] = motor::memory::global_t::alloc( tcp_client_data(),
        "[unix_net_module] : tcp data" ) ;

    return id ;
}

//*****************************************************************
size_t unix_net_module::create_tcp_server_id( SOCKET s ) noexcept
{
...
//*****************************************************************
void_t unix_net_module::release_all_tcp( void_t ) noexcept
{
