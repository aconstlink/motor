...
public:
    ...
    motor::device::components::button_state check_left_trigger(
        XINPUT_STATE const& new_state, uint16_t& intensity_out ) const noexcept
    {
        ...
        if( new_press && !old_press ) return motor::device::components::button_state::pressed ;
        if( new_press && old_press ) return motor::device::components::button_state::pressing ;
        if( old_press && !new_press ) return motor::device::components::button_state::released ;

        return motor::device::components::button_state::none ;
    }

    /// check the state of the right trigger
    motor::device::components::button_state check_right_trigger(
        XINPUT_STATE const& new_state, uint16_t& intensity_out ) const noexcept
    {
    ...
    motor::device::components::stick_state check_left_stick(
        XINPUT_STATE const& new_state, motor::math::vec2f_t& nnc_out ) const noexcept
    {
        ...
        if( !old_tilt.any() && new_tilt.any() ) return motor::device::components::stick_state::tilted ;
        if( old_tilt.any() && new_tilt.any() ) return motor::device::components::stick_state::tilting ;
        if( old_tilt.any() && !new_tilt.any() ) return motor::device::components::stick_state::untilted ;

        return motor::device::components::stick_state::none ;
    }
    ...
    motor::device::components::stick_state check_right_stick(
        XINPUT_STATE const& new_state, motor::math::vec2f_t& nnc_out ) const noexcept
    {
        motor::math::vec2b_t const old_tilt = motor::math::vec2b_t(
...
namespace this_file
{
    typedef motor::device::layouts::xbox_controller_t::input_component ic_t ;
    ...
    typedef motor::device::layouts::xbox_controller_t::button button_t ;
    ...
    typedef motor::device::layouts::xbox_controller_t::thumb thumb_t ;
    ...
    typedef motor::device::layouts::xbox_controller_t::shoulder shoulder_t ;
    ...
    typedef motor::device::layouts::xbox_controller_t::dpad dpad_t ;
...
//***
void_t xinput_module::search( motor::device::imodule::search_funk_t funk )  noexcept
{
    for( auto & d : _devices )
    {
...
void_t xinput_module::update( void_t ) noexcept 
{
    ...
    for( auto& item : _devices )
    {
        ...
        motor::device::xbc_device_mtr_t dev = item.dev ;
        ...
        // buttons : Just the on/off buttons
        {
            ...
            for( auto const & i : buttons__ )
            {
                ...
                {
                    *ctrl.comp_button( b ) = motor::device::components::button_state::pressed ;
                    *ctrl.comp_button( b ) = 1.0f ;
                }
                else if( helper.is_pressing( state, i ) )
                {
                    *ctrl.comp_button( b ) = motor::device::components::button_state::pressing ;
                    *ctrl.comp_button( b ) = 1.0f ;
                }
                else if( helper.is_released(state, i ) )
                {
                    *ctrl.comp_button( b ) = motor::device::components::button_state::released ;
        ...
        // triggers
        {
            uint16_t intensity = 0 ;
            {
                ...
                {
                    *ctrl.get_component( motor::device::layouts::xbox_controller_t::trigger::left ) = bs ;
                    *ctrl.get_component( motor::device::layouts::xbox_controller_t::trigger::left ) =
            ...
            }
            {
                ...
                if( bs != motor::device::components::button_state::none )
                {
                    *ctrl.get_component( motor::device::layouts::xbox_controller_t::trigger::right ) = bs ;
                    *ctrl.get_component( motor::device::layouts::xbox_controller_t::trigger::right ) =
                        float_t( intensity ) / 255.0f ; ;
                }
            }
        }
        ...
        // sticks
        {
            motor::math::vec2f_t change ;
            {
                ...
                if( ss != motor::device::components::stick_state::none )
                {
                    *ctrl.get_component( motor::device::layouts::xbox_controller_t::stick::left ) = ss ;
                    *ctrl.get_component( motor::device::layouts::xbox_controller_t::stick::left ) = change ;
                }
            }

            {
                ...
                if( ss != motor::device::components::stick_state::none )
                {
                    *ctrl.get_component( motor::device::layouts::xbox_controller_t::stick::right ) = ss ;
                    *ctrl.get_component( motor::device::layouts::xbox_controller_t::stick::right ) = change ;
                }
            }
        }
        ...
        // motors - this is output to the controller
        {
            ...
            {
                auto * motor = ctrl.get_component( motor::device::layouts::xbox_controller_t::vibrator::left ) ;
                vib.wLeftMotorSpeed = WORD( 65535.0f * motor->value() ) ;
            }

            {
                auto* motor = ctrl.get_component( motor::device::layouts::xbox_controller_t::vibrator::right ) ;
