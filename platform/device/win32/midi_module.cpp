...
//****************************************************************************************
void_t midi_module::install( motor::device::iobserver_mtr_t ptr ) noexcept 
{
    if( motor::device::midi_observer_ptr_t ptr_ = dynamic_cast<motor::device::midi_observer_ptr_t>( ptr ); ptr_!= nullptr  )
    {
        std::lock_guard< std::mutex > lk( _mtx_observers ) ;
        auto iter = std::find_if( _observers.begin(), _observers.end(), [&]( motor::device::midi_observer_mtr_t ptr_in )
        {
            return ptr_in == ptr_ ;
        } ) ;
...
void_t midi_module::unregister_device( midi_module::device_data_ref_t item, bool_t const closed ) noexcept
{
    if( item.mdata.inh != NULL )
    {
        ...
        // midiInAddBuffer used here.
        {
            ...
            motor::log::global::error( res != MMSYSERR_NOERROR,
                "[motor::device::win32::midi_module::unregister_device] : midiInReset" ) ;
        }

        {
            ...
            motor::log::global::error( res != MMSYSERR_NOERROR,
                "[motor::device::win32::midi_module::unregister_device] : midiInUnprepareHeader" ) ;
        ...
        }
        {
            ...
            motor::log::global::error( res != MMSYSERR_NOERROR,
                "[motor::device::win32::midi_module::unregister_device] : midiInClose" ) ;
        }
        item.mdata.inh = NULL ;
    }
    ...
    if( item.mdata.outh != NULL )
    {
        ...
        if( !closed )
        {
            ...
            motor::log::global::error( res != MMSYSERR_NOERROR,
                "[motor::device::win32::midi_module::unregister_device] : sending -1 failed" ) ;
        }
...
void_t midi_module::update( void_t ) noexcept
{
    ...
    for( auto & item : _devices )
    {
        ...
        #if 0
        motor::device::midi_device_t::midi_messages_t msgs ;
...

        {
            ...
            motor::log::global_t::error( res != MMSYSERR_NOERROR,
                "[motor::device::win32::midi_module::update] : midiInReset" ) ;
        }

        {
            auto res = midiInStart( item.mdata.inh ) ;
            motor::log::global::error( res != MMSYSERR_NOERROR,
                "[motor::device::win32::midi_module::update] : midiInStart" ) ;
        }
...
//****************************************************************************************
void_t midi_module::handle_message( HMIDIIN hin, motor::device::midi_message_cref_t msg ) noexcept
{
    for( auto & item : _devices )
    {
...
void_t midi_module::create_devices( void_t ) noexcept
{
    this_file::for_each_device( [&]( uint_t const idx, MIDIINCAPS const & caps )
    {
        ...
        {
            motor::device::midi_device_t mdev = motor::device::midi_device_t( motor::string_t( caps.szPname ) ) ;
...
void_t midi_module::open_midi_in( uint_t const idx, midi_data_out_t md ) noexcept 
{
    ...
    motor::log::global::warning( res != MMSYSERR_NOERROR, 
        "[motor::device::midi_module::check_handle_for_device] : \
    ...
    if( res == MMSYSERR_NOERROR )
    {
        ...
        midihdr.lpData = (LPSTR)motor::memory::global_t::alloc_raw<uchar_t>(10, 
            "[motor::device::win32::midi_module::check_handle_for_device] : MIDIHDR.lpData" ) ;
        midihdr.dwFlags = 0 ;
        ...
        motor::log::global::error( prep_res != MMSYSERR_NOERROR, 
            "[motor::device::midi_module::check_handle_for_device] : midiInPrepareHeader" ) ;

        auto const add_res = midiInAddBuffer( hmin, &midihdr, sizeof( MIDIHDR ) ) ;
        motor::log::global::error( prep_res != MMSYSERR_NOERROR, 
            "[motor::device::midi_module::check_handle_for_device] : midiInAddBuffer" ) ;
