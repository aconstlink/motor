...
//
// @obsolete do not use it anymore. initialization is done somewhere
// else without the window id.
class init_msl_visitor : public default_visitor
{
    motor_this_typedefs( init_msl_visitor );
  ...
  public:
    ...
    init_msl_visitor( motor::application::window_id_t const wid,
    ...
    init_msl_visitor( this_rref_t rhv ) noexcept
        : _wid( rhv._wid ), _id( rhv._id ), _fe( motor::move( rhv._fe ) )
    {
    }
    init_msl_visitor( this_cref_t ) = delete;
    virtual ~init_msl_visitor( void_t ) noexcept {}
...
motor_typedef( init_msl_visitor );
} // namespace scene
} // namespace motor