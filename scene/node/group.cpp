...
motor::scene::result group::apply( motor::scene::ivisitor_ptr_t vptr ) noexcept
{
    ...
    {
    case motor::scene::ok:
        r = this_t::traverse_children( vptr, _funk );
        break;
    case motor::scene::result::break_traversal:
        break;
    default:
        break;
    }
...
{
    if( with_ptr == nullptr ) return motor::scene::invalid_argument;

    if( which_ptr == with_ptr ) return motor::scene::ok;

    auto found = std::find( _children.begin(), _children.end(), which_ptr );
    if( found == _children.end() ) return motor::scene::invalid_argument;
...
{
    if( which_ptr == nullptr ) return motor::scene::ok;
...
{
    if( child_i >= _children.size() ) return nullptr;
...
{
    if( nptr == nullptr ) return size_t( -1 );
...
{
    if( nptr == nullptr || nptr == this ) return motor::scene::invalid_argument;
...
{
    auto found = std::find( _children.begin(), _children.end(), nptr );
    if( found == _children.end() ) return size_t( -1 );
...
//*******************************************************************
motor::scene::result group::traverse_children( motor::scene::ivisitor_ptr_t ptr ) noexcept
{
    return this_t::traverse_children( ptr, _funk );
}

//*******************************************************************
motor::scene::result group::traverse_children(
    motor::scene::ivisitor_ptr_t ptr, traverse_predicate_t func ) noexcept
{
    motor::scene::result r = motor::scene::result::ok;

    for( size_t i = 0; i < _children.size(); ++i )
    {
        if( !func( i ) ) continue;

        r = motor::scene::repeat;
        ...
        if( r == motor::scene::result::break_traversal ) break;
    }
