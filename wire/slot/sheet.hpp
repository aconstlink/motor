...
namespace motor
{
    namespace wire
    {
        template< typename T, bool_t named_t >
        class sheet ;
        ...
        using named_inputs_t = sheet< motor::wire::iinput_slot, true > ;
        using named_outputs_t = sheet< motor::wire::ioutput_slot, true > ;

        motor_typedefs( named_outputs_t, outputs ) ;
        motor_typedefs( named_inputs_t, inputs ) ;


        // this class is used for storing 
        // signals/slots without a name
        // usage: 
        // unnamed_sheet< motor::wire::islot >
        template< typename T >
        class sheet< T, false >
        {
            using t_ptr_t = T * ;
            using __this_t = sheet< T, false > ;
            motor_this_typedefs( __this_t ) ;

        private:

            bool_t _disconnect_on_clear = true ;
            motor::vector< t_ptr_t > _ts ;
        ...
        public:
            ...
            // add signal
            bool_t add( motor::core::mtr_safe< T > && other ) noexcept 
            {
                _ts.emplace_back( motor::move( other ) ) ;
        ...
        using unnamed_inputs_t = sheet< motor::wire::iinput_slot, false > ;
        using unnamed_inputs_ref_t = unnamed_inputs_t & ;

        using unnamed_outputs_t = sheet< motor::wire::ioutput_slot, false > ;
        using unnamed_outputs_ref_t = unnamed_outputs_t & ;
