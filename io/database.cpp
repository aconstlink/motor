...
static const motor::string_t header_desc = "natus db file format 0.1" ;
...
database::file_record::~file_record( void_t ) noexcept
{
    motor::memory::release_ptr( cache ) ;
}
struct database::record_cache
{
    ...
    record_cache( void_t ) noexcept
    {
    }

    record_cache( database* owner_new, size_t const idx ) noexcept
    {
        owner = owner_new ;
        _idx = idx ;
    }

    record_cache( this_rref_t rhv ) noexcept
    {
        *this = std::move( rhv ) ;
    }
    ...
    record_cache::this_ref_t operator = ( this_rref_t rhv ) noexcept
    {
        _idx = rhv._idx ;
        rhv._idx = size_t( -1 ) ;
        owner = rhv.owner ;
    ...
    void_t take_load_handle( motor::io::load_handle_rref_t hnd )
    {
        _lh = std::move( hnd ) ;
    }
    ...
    motor::io::database::record_cache::record_cache_mtr_t load(
        motor::io::location_cref_t loc, bool_t const reload ) noexcept
    {
        return owner->load( loc, reload )._res ;
    }

    motor::io::database::record_cache::record_cache_mtr_t load( bool_t const reload ) noexcept
    {
...
//***
database::database( void_t ) noexcept
{
}

//***
database::database( motor::io::path_cref_t base, motor::io::path_cref_t name,
    motor::io::path_cref_t working_rel ) noexcept
{
...
//***
bool_t database::init( motor::io::path_cref_t base, motor::io::path_cref_t working, motor::io::path_cref_t name ) noexcept
{
    this_t::db_t db ;
    ...
    {
        motor::log::global_t::warning( motor::from_std( "Path does not exist : " + motor::io::path_t( base / working.string() ).string() ) ) ;
    }
...
//***
bool_t database::pack( this_t::encryption const ) noexcept
{
    ...
    // write file
    {
        ...
        // header info
        {
            ...
            motor::string_t const si = this_t::obfuscator().
                encode( header_desc + motor::from_std( ":" + std::to_string( _db.records.size() ) + ": ") ) ;
...
//***
void_t database::load_db_file( this_t::db_ref_t db_, motor::io::path_cref_t p ) noexcept
{
    ...
    // check file header description
    {
        ...
        if( num_elems >= 3 )
        {
            ...
            {
                motor::log::global_t::error( "[db] : Invalid db file. Header description mismatch. Should be " + header_desc + " for file " + motor::from_std( p.string() ) ) ;
                motor::log::global_t::error( "[db] : Just repack the data with the appropriate packer version." ) ;
                return ;
            }

            num_records = std::stol( std::string(token[ 1 ]) ) ;
        }
        else 
        {
    ...
    // add file records
    {
        for( size_t i = 0; i < num_records; ++i )
        {
            ...
            fr.offset = std::stol( std::string(token[ 2 ]) ) ;
            fr.sib = std::stol( std::string(token[ 3 ]) ) ;
            fr.cache = motor::memory::create( this_t::record_cache( const_cast< this_ptr_t >( this ), db_.records.size() ) ) ;
...
//***
bool_t database::unpack( void_t ) noexcept
{
    return true ;
}

//***
motor::io::store_handle_t database::store( motor::io::location_cref_t loc, char_cptr_t d, size_t const s ) noexcept
{
    auto p  = _db.working ;
...
//***
database::cache_access_t database::load( motor::io::location_cref_t loc, bool_t const reload ) noexcept
{
    this_t::file_record_t fr ;
    if( !this_t::lookup( loc, fr ) )
    {
        motor::log::global_t::warning( "resource location not found : " + loc.as_string() ) ;
        return this_t::cache_access_t( this_t::record_cache_mtr_t( nullptr ) ) ;
    }
    ...
    {
        motor::io::load_handle_t lh ;
        
        // load from filesystem
        if( fr.offset == uint64_t( -2 ) )
        {
            lh = motor::io::global_t::load( _db.working / fr.rel, motor::io::obfuscator_t() ) ;
        }
        // load from .mdb file
        else if( fr.offset != uint64_t( -1 ) )
        {
            ...
            lh = motor::io::global_t::load( p, offset, fr.sib, this_t::obfuscator() ) ;
        }
...
void_t database::detach( motor::string_cref_t loc, motor::io::monitor_mtr_t mon ) noexcept 
{
    ...
    if( this_t::lookup( _db, loc, fr ) )
    {
        auto iter = std::remove_if( fr.monitors.begin(), fr.monitors.end(),
            [&] ( motor::io::monitor_mtr_t const& r )
        {
            return r == mon ;
        } ) ;
...
void_t database::attach( motor::io::monitor_mtr_t moni ) noexcept 
{
    ...
    auto iter = std::find_if( _db.monitors.begin(), _db.monitors.end(), 
        [&]( motor::io::monitor_mtr_t const & m )
    {
        return m == moni ;
    } ) ;
...
database::file_record_t database::create_file_record( this_t::db_ref_t db, motor::io::path_cref_t path ) const noexcept
{
    ...
    {
        fr.cache = motor::memory::create( this_t::record_cache( const_cast< this_ptr_t >( this ), db.records.size() ) ) ;
    }
...
//***
bool_t database::lookup( this_t::db const & db_, motor::io::location_cref_t loc, file_record_out_t fro ) const noexcept
{
...
{
    motor::concurrent::mrsw_t::writer_lock_t lk( _ac ) ;
    auto iter = std::find_if( _db.records.begin(), _db.records.end(), 
        [&]( this_t::file_record_cref_t fr )
    {
...
void_t database::file_remove( this_t::file_record_cref_t fri ) noexcept 
{
    ...
    auto iter = std::remove_if( _db.records.begin(), _db.records.end(), 
        [&] ( this_t::file_record_cref_t fr )
    {
        return fr.location == fri.location ;
    } ) ;
...
//***
motor::io::location_t database::location_for_index( size_t const idx ) const noexcept
{
    motor::concurrent::mrsw_t::reader_lock_t lk( _ac ) ;
...
{
    auto iter = std::find_if( db.records.begin(), db.records.end(), 
        [&] ( this_t::file_record_cref_t fr )
    {
        return fr.location == fri.location ;
    } ) ;
