        ...
        {
            {
                ...
                {
                    "return fract ( sin(n) * 43983.5453123 ) ;",
                }
            },
            {
                // motor::msl::signature_t
            ...
            },
            {
                ...
                // fragmetns_t/strings_t
                {
                    "float f1 = floor ( x ) ;",
                    "float fc = fract ( x ) ;",
                    this_file_essl3::replace_buildin_symbols("return lerp ( :rand_1: ( f1 ) , :rand_1: ( f1 + 1.0 ) , fc ) ;" )                },
            },
            {
                // motor::msl::signature_t
                { 
                ...
                // fragmetns_t/strings_t
                {
                    ...
                    "u = u * u * ( 3.0 - 2.0 * u ) ;",
                    "float res = mix( ",
                    this_file_essl3::replace_buildin_symbols( "mix ( :rand_1: ( ip ) , :rand_1: ( ip + vec2 ( 1.0 , 0.0 ) ) , u.x )," ),
                    this_file_essl3::replace_buildin_symbols( "mix( :rand_1: ( ip+vec2(0.0,1.0) ), :rand_1: ( ip+vec2(1.0,1.0)),u.x ),u.y);"),
                    "return res*res;"
                },
            },
            {
...
namespace this_file_essl3
{
    ...
    {
        return this_file_essl3::map_variable_type( apit, type ).second ;
    }
...
motor::string_t essl3_generator::replace_types( motor::msl::api_type const apit, motor::string_t code ) noexcept
{
    ...
    while( p1 != std::string::npos )
    {
        ...
        {
            code.replace( p0, dist, this_file_essl3::map_variable_type_to_string( apit, t ) ) ;
        }
        p0 = p1 + 1 ;
...
//******************************************************************************************************************************************
motor::msl::generated_code_t::shaders_t essl3_generator::generate( motor::msl::generatable_cref_t genable_, motor::msl::variable_mappings_cref_t var_map_ ) noexcept
{
    ...
    // replace buildins
    {
        for( auto& s : genable.config.shaders )
        {
            for( auto& c : s.codes )
            {
                for( auto& l : c.lines )
                {
                    l = this_file_essl3::replace_buildin_symbols( std::move( l ) ) ;
                }
            }
        }
        ...
        for( auto& frg : genable.frags )
        {
            for( auto& f : frg.fragments )
            {
                //for( auto& l : c.lines )
                {
                    f = this_file_essl3::replace_buildin_symbols( std::move( f ) ) ;
                }
            }
        }
    }
    ...
    // inject composite buildins
    {
        ...
        for( auto iter = tmp.begin(); iter != tmp.end(); ++iter )
        {
            ...
            if( !this_file_essl3::buildin_by_opcode( test_bi, buildins_ ) ) continue ;
    ...
    motor::msl::variable_mappings_cref_t var_mappings, motor::msl::api_type const type ) noexcept
{
        ...
        case motor::msl::api_type::es3:
            ...
            // print down all precision declarations 
            {
                ...
                for( auto const & e : table )
                {
                    ...
                    {
                        auto const t = this_file_essl3::map_variable_type( type, v.type ) ;
                        if( t.first == e )
                        {
    ...
    if( sht_cur == motor::msl::shader_type::geometry_shader )
    {
        for( auto const & pd : shd_.primitive_decls )
        {
            ...
            {
                text << "layout ( " << this_file_essl3::input_decl_type_to_string(pd.pdt) << " ) in ;" << std::endl ;
            }
            else if( pd.fq == motor::msl::flow_qualifier::out )
            {
                text << "layout ( " << this_file_essl3::output_decl_type_to_string(pd.pdt) << ", max_vertices = " << 
    ...
    // the prototype help with not having to sort funk definitions
    {
        ...
        {
            text << this_file_essl3::map_variable_type_to_string( type, f.sig.return_type ) << " " ;
            text << f.sym_long.expand( "_" ) << " ( " ;
            for( auto const& a : f.sig.args )
            {
                text << this_file_essl3::map_variable_type_to_string( type, a.type ) + ", " ;
            }
    ...
    // 3. make all functions with replaced symbols
    {
        ...
        for( auto const& f : genable.frags )
        {
            // make signature
            {
                ...
                for( auto const& a : f.sig.args )
                {
                    text << this_file_essl3::map_variable_type_to_string( type, a.type ) + " " + a.name + ", "  ;
                }
                text.seekp( -2, std::ios_base::end ) ;
                text << " )" << std::endl ;
            }
        ...
        // in/out interface block
        {
            // all ins
            {
                ...
                for( auto & v : shd_.variables )
                {
                    ...
                    motor::string_t const type_ = this_file_essl3::map_variable_type_to_string( type, v.type ) ;
