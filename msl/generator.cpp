...
//*******************************************************************************
motor::msl::generated_code_t generator::generate_old( void_t ) noexcept
{
    motor::msl::generated_code_t ret ;
    ...
    // streamout type
    {
        ...
        if( !has_pixel_shader ) ret.streamout = motor::msl::streamout_type::interleaved ; 
    }

    motor::msl::generated_code_t::shaders_t shaders ;
    // glsl 
    {
        auto const shds = motor::msl::glsl::generator_t().generate( _genable, mappings ) ;
        for( auto shd_ : shds )
        {
    ...
    // hlsl
    {
        auto const shds = motor::msl::hlsl::generator_t().generate( _genable, mappings ) ;
        for( auto shd_: shds )
        {
            shaders.emplace_back( shd_ ) ;
        }
