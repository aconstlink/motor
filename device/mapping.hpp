...
namespace motor
{
    namespace device
    {
        ...
        namespace detail
        {
            using comp_t = motor::device::icomponent ;
            using comp_button_t = motor::device::components::button_t ;
            using comp_ascii_t = motor::device::layouts::ascii_keyboard_t::ascii_component_t;
            //using comp_knob_t = motor::device::components::knob_t ;
            using comp_point_t = motor::device::components::point_t ;
            using comp_scroll_t = motor::device::components::scroll_t ;
            //using comp_slider_t = motor::device::components::slider_t ;
            using comp_stick_t = motor::device::components::stick_t ;
            //using comp_touch_t = motor::device::components::touch_t ;


            //using comp_led_t = motor::device::components::led_t ;
            //using comp_light_t = motor::device::components::light_t ;
            using comp_motor_t = motor::device::components::vibrator_t ;

            typedef std::function< void_t ( comp_t*, comp_t const* ) > mapping_funk_t ;

            template< typename T >
            bool_t b_to_a( comp_t*, comp_t*, motor::device::mapping_detail const, mapping_funk_t& ) 
            {
                return false ;
            }
            ...
            bool_t b_to_a<comp_stick_t>( comp_t* /*ic_a*/, comp_t* ic_b, 
                motor::device::mapping_detail const det, mapping_funk_t& funk_out )
            {
                ...
                // stick to stick
                {
                    ...
                    if( comp != nullptr )
                    {
                        funk_out = [&] ( comp_t* a_, comp_t const* b_ )
                        {
                            ...
                            if( b->state() != motor::device::components::stick_state::none ) 
                                *a = *b ;
                        } ;
                        return true ;
                    }
                ...
                // point to stick
                {
                    ...
                    if( comp != nullptr )
                    {
                        funk_out = [&] ( comp_t* a_, comp_t const* b_ )
                        {
                            ...
                            if( b->has_changed() )
                            {
                                motor::device::components::stick_state ss = motor::device::components::stick_state::none ;

                                switch( b->state() )
                                {
                                case motor::device::components::move_state::moved: 
                                    ss = motor::device::components::stick_state::tilted ; break ;
                                case motor::device::components::move_state::moving:
                                    ss = motor::device::components::stick_state::tilting ; break ;
                                case motor::device::components::move_state::unmoved:
                                    ss = motor::device::components::stick_state::untilted ; break ;
                                default: break ;
                                }
                ...
                // ascii to stick
                {
                    ...
                    if( comp != nullptr )
                    {
                        funk_out = [=] ( comp_t* a_, comp_t const* b_ )
                        {
                            ...
                            if( b->state() != motor::device::components::key_state::none )
                            {
                                motor::device::components::stick_state ss = motor::device::components::stick_state::none ;
                                switch( b->state() ) 
                                {
                                case motor::device::components::key_state::pressed: 
                                    ss = motor::device::components::stick_state::tilted; break ;
                                case motor::device::components::key_state::pressing: 
                                    ss = motor::device::components::stick_state::tilting; break ;
                                case motor::device::components::key_state::released: 
                                    ss = motor::device::components::stick_state::untilted; break ;
                                default: break ;
                                }
                                ...
                                switch( det )
                                {                                
                                case motor::device::mapping_detail::negative_x: v0.x( -v1 ) ; break ;
                                case motor::device::mapping_detail::positive_x: v0.x( +v1 ) ; break ;
                                case motor::device::mapping_detail::negative_y: v0.y( -v1 ) ; break ;
                                case motor::device::mapping_detail::positive_y: v0.y( +v1 ) ; break ;
                                case motor::device::mapping_detail::one_to_one: // not possible
                                default: 
            ...
            bool_t b_to_a<comp_point_t>( comp_t* /*ic_a*/, comp_t* ic_b, 
                motor::device::mapping_detail const, mapping_funk_t& funk_out )
            {
                ...
                // button to ascii
                {
                    ...
                    if( comp != nullptr )
                    {
                        funk_out = [&] ( comp_t* a_, comp_t const* b_ )
                        {
                            ...
                            motor::device::components::key_state bs = components::key_state::none ;

                            switch( b->state() )
                            {
                            case motor::device::components::button_state::pressed: bs = components::key_state::pressed ; break ;
                            case motor::device::components::button_state::pressing: bs = components::key_state::pressing ; break ;
                            case motor::device::components::button_state::released: bs = components::key_state::released ; break ;
                            default: break ;
                            }
                ...
                // button to button
                {
                    ...
                    if( comp != nullptr )
                    {
                        funk_out = [&] ( comp_t* a_, comp_t const* b_ )
                        {
                            a_t* a = reinterpret_cast< a_t* >( a_ ) ;
                            b_t const* b = reinterpret_cast< b_t const* >( b_ ) ;
                            if( b->state() != motor::device::components::button_state::none )
                                *a = *b ;
                ...
                // ascii key to button
                {
                    ...
                    if( comp != nullptr )
                    {
                        funk_out = [&] ( comp_t* a_, comp_t const* b_ )
                        {
                            ...
                            motor::device::components::button_state bs = components::button_state::none ;

                            switch( b->state() )
                            {
                            case motor::device::components::key_state::pressed: bs = components::button_state::pressed ; break ;
                            case motor::device::components::key_state::pressing: bs = components::button_state::pressing ; break ;
                            case motor::device::components::key_state::released: bs = components::button_state::released ; break ;
                            default: break ;
                            }
        ...
        public:
            ...
            bool_t insert( 
                ...
                motor::device::mapping_detail const det = mapping_detail::one_to_one ) noexcept
            {
